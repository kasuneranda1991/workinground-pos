...
//click confirm start
$('#confirmationTable tbody').on('click', 'button.confirmButton', function (e) {
	console.log('confirmed');
	var data = tableData.row( $(this).parents('tr') ).data();
	$('#confirm_payment_id').val(data.id);
  $('#confirmtype').val('confirmed');
	var form = $('#reservationConfirmForm');
...
$('#confirmationTable tbody').on('click', 'button.rejectmButton', function () {
  ...
  $('#rejectType').val('rejected');
	});
//click reject end
$('#rejectButton').click(function(e){
$('#rejectModel').modal('hide');
var form = $('#reservationRejectForm');
var url = form.attr('action');
$.ajax({
     type: "POST",
     url: url,
