...
<div class="col-lg-12 col-sm-12 mb-12">
  ...
  <div class="card-body">
    ...
    <p class="card-text d-inline-block mb-3">{{$job->remark}} given by {{ $customer->name }} who lives on {{ $customer->address }}.<br>Job Completed with in {{ Carbon\Carbon::parse($job->created_at)->diffInDays($job->updated_at) }} </p><br>
    <strong>{{ $customer->contact_no }} | {{ $customer->nic }}</strong><br><hr>
    <span class="text-muted">{{ $job->created_at->toFormattedDateString() }} - {{ $job->created_at->diffForHumans() }}</span><span class="float-right"> <input class="btn btn-white" type="button" value="Rs.{{$job->job_price}}" ></span>
    @endif
    @endforeach
  </div>
  </div>
